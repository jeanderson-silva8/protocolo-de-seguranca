...
### [`AUDIT_REPORT_Miniatura-Forja-AI_2026-05-23.md`](AUDIT_REPORT_Miniatura-Forja-AI_2026-05-23.md)

Auditoria de um **micro-SaaS de geração de thumbnails via IA** — Miniatura Forja AI (React 19 + Vite 8 frontend · Node + Express dev server + Vercel Serverless Function em prod · `@fal-ai/client` FLUX Dev como motor de imagem). Endpoint público sem autenticação que dispara chamadas pagas a uma API de IA — caso clássico de "DoS econômico" se o rate limit falhar. **Resultado (MODO PARANOICO, 2 passadas integradas):** ✅ 5 sólidos · 🟡 8 · 🟠 7 · **🔴 1 crítico** · 🚫 24 N/A por escopo (sem auth, sem banco, sem cookies). O achado crítico **não é um bug de execução remota**: é **`FAL_KEY` real publicada no histórico público do Git** (commit `f5f724a` adicionou `.env`, commit `d402e7d` removeu do tracking mas o histórico preserva — `git log --all -p -- .env` num clone público entrega a chave). Remediação real exige rotação na fal.ai, não basta limpar o tracking. Os 7 🟠 desenham o padrão estrutural: **`server.js` (dev) ≠ `api/generate.js` (prod)** — 5 divergências entre os dois entrypoints (Helmet, body limit, threshold de rate limit, CORS bloqueante, mensagem de erro). Combinado com `x-forwarded-for` lido cru (bypass do rate limit em 1 linha de curl) + store in-memory que zera no cold start + ausência de `vercel.json` (frontend sem CSP/HSTS), o vetor concreto é **drenar a conta paga da fal.ai** via burst + rotação de header. A Passada 2 reclassificou 1 🟠→🟡 (Helmet em endpoint JSON-only tem impacto isolado baixo) e descobriu 2 achados novos: `req.headers['x-forwarded-for']` pode vir como **array** em proxies múltiplos (rate limit vira inútil), e CORS em prod **não rejeita** origem não-allowlist — só omite o header. **Nenhuma pergunta nova promovida** ao framework (todos os achados caem em itens existentes — 13, 17, 37, 41, 58); registrada **proposta de expansão do item 58** para cobrir explicitamente o histórico do git como vetor distinto do `dist/`. **Lição meta:** comentários `[SEGURANÇA]` no código não são mitigação — o `server.js` cheio de prefixos "Enterprise" é **dev-only**, e o handler de prod (`api/generate.js`) que serve o tráfego implementa a metade do que o dev tem. Padrão TrendScope C2 repetido (3ª vez consecutiva); reforça empiricamente que o item 17 do checklist (multi-entrypoint) é a classe de bug mais frequente em apps Vercel-style.

### [`AUDIT_REPORT_Miniatura-Forja-AI_v2_2026-05-23.md`](AUDIT_REPORT_Miniatura-Forja-AI_v2_2026-05-23.md)

Segunda passada sobre o Miniatura Forja AI (modo comparativo, padrão FlowSnyker v2 / Lumina v2), **verificando se as 15 correções da v1 estavam de fato em produção**. A v1 marcou todas as correções como aplicadas no código — corretamente, o repo está limpo. **A v2 abriu a URL ao vivo e descobriu que NADA das correções de runtime estava em produção** — o bundle JavaScript servido em `thumbnail-forge-one.vercel.app` ainda continha a copy antiga ("3/3 grátis hoje" em vez de "5 grátis por minuto"), o `/api/generate` não setava nenhum dos headers manuais (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`), o CORS não bloqueava origens não-allowlist (testei com `Origin: https://evil.com` → HTTP 200 + 2 imagens geradas, ou seja, gastei $0.10 reais provando o achado), e o `vercel.json` com CSP/HSTS/`X-Frame-Options` não estava aplicando headers. Causa diagnosticada: a integração GitHub→Vercel está vinculada ao repo antigo (`silvajeanderson165-creator/thumbnail-forge`), não ao novo (`jeanderson-silva8/Miniatura-Forja-AI`) — o `git remote set-url` mudou o destino do push local mas não reconfigurou o webhook da Vercel. **Achados novos da v2:** (V2-A) re-introdução do segredo via relatório de auditoria — o leak `FAL_KEY=9e3137f5-...` foi documentado em `docs/AUDIT_REPORT_2026-05-23.md` como evidência reproduzível e o próprio relatório re-introduziu a string no repo (chave já revogada, então o vetor é inerte, mas a lição metodológica vale); (V2-B) deploy desincronizado — código novo no repo correto, deploy ainda no repo antigo; (V2-C) `Access-Control-Allow-Origin: *` default da Vercel em assets estáticos (não-vuln mas vale flag). **Lição meta:** auditar o repositório ≠ auditar o produto. A entrega da v1 deveria ter terminado com `curl` no domínio publicado, não com `git log` confirmando que os arquivos foram commitados. **Proposta documentada de expansão do item 17 do checklist** com sub-pergunta de "deploy hygiene": "quando você diz 'corrigi', você verificou via curl na URL pública que o header novo está sendo servido?". Próximo passo: v3 após reconexão do webhook Vercel.

### [`AUDIT_REPORT_Miniatura-Forja-AI_v3_2026-05-23.md`](AUDIT_REPORT_Miniatura-Forja-AI_v3_2026-05-23.md)

Terceira passada sobre o Miniatura Forja AI (confirmação curta de runtime pós-redeploy). Depois que a v2 detectou que o deploy estava ligado ao repo antigo, o autor excluiu o projeto da Vercel e reconectou ao `jeanderson-silva8/Miniatura-Forja-AI`. **A v3 rodou os mesmos `curl` da v2 esperando o resultado oposto** — e confirmou: 5/5 testes de runtime passaram. Headers do `vercel.json` aplicados em `/` (CSP, HSTS, X-Frame-Options, Permissions-Policy, etc.); headers manuais de `setSecurityHeaders()` aplicados em `/api/generate` (nosniff, DENY, no-referrer, no-store); CORS bloqueante com `Origin: https://evil.com` agora retorna **HTTP 403 `{"error":"Origin não autorizada."}`** (era HTTP 200 + 2 imagens geradas na v2); bundle JS atualizado para `index-dv1S03mm.js` contendo a copy nova `"5 grátis por minuto"` (era `index-PInybKud.js` com `"3/3 grátis hoje"` na v2). **Veredito final do ciclo:** 15/15 achados de runtime confirmados em produção. 2 mantidos como dívida cosmética conscientemente aceita. 0 regressões. Resta apenas V2-A (chave revogada citada em texto claro em `docs/AUDIT_REPORT_2026-05-23.md` — vetor inerte porque a chave foi revogada; redação é decisão do autor). **Lição meta fechada:** o ciclo v1 → v2 → v3 (auditoria → comparativa → confirmação) custa 3 leituras mas estabelece o que realmente importa medir — não "código novo está commitado", mas "header novo aparece no `curl`". Reforça a proposta de expansão do item 17 do checklist com sub-pergunta de "deploy hygiene": quando uma auditoria declara correção como ✅, a evidência deve incluir verificação na URL publicada, não só no repositório. Projeto Miniatura Forja AI agora **pronto para portfólio** — auditado, endurecido, validado em runtime, ciclo de método completo no histórico.
...
## 🚀 Como Executar e Consultar Localmente

Este repositório serve como central de auditorias de segurança, modelos de ameaças (STRIDE) e changelogs de projetos desenvolvidos. Não há código executável ou dependências a serem instaladas.

### Requisitos
- Um visualizador de arquivos Markdown (como VS Code com extensão Markdown Preview, Obsidian ou GitHub Desktop)

### Utilização

Você pode navegar pelos relatórios e listas de verificação simplesmente abrindo os arquivos `.md` no seu editor. Para visualizar os relatórios estruturados com tabelas e formatação idêntica à do GitHub:
- No VS Code, abra qualquer arquivo `.md` e pressione `Ctrl + Shift + V` para habilitar a visualização renderizada.
